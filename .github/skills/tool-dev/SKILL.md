...
- `toolkit-root` — workspace-relative path the entry prompt resolved (`.copilot-toolkit/build/.github` when consumed via submodule, `build/.github` when self-hosted in this repo). The skill threads this value to every subagent so each one can locate its `{toolkit-root}/skills/tool-dev/...` references at runtime.
...
**BEFORE any file write, tool call, or terminal command, you MUST create a todo list using `manage_todo_list` that mirrors the chosen action's step list verbatim (For "create": 6 steps; For "update": 6 steps; For "review": 7 steps).** Mark in-progress → completed one at a time. The task is done **only when every todo is completed** — do not stop after the last code edit. No exceptions.

## Rules

- **No wildcard tools** — always specify exact tool names, never use `*`
- **Recipe glue = Node script, not inline shell** — deterministic multi-step glue in a recipe file (git orchestration, JSON shaping, file scaffolding) goes in a committed `scripts/<name>.mjs` run as `node .copilot-toolkit/build/scripts/<name>.mjs …`, NOT an inline `pwsh`/`bash` block in the markdown. Inline shell in recipes breaks on cross-platform quoting/escaping and can't be tested; Node is the guaranteed runtime (preflight). Single host commands (`git --no-pager …`) stay inline; anything with a loop / branch / multi-command sequence becomes a script. LLM judgment stays in markdown — only deterministic glue moves to the script. The built recipe gate fails on multi-step inline shell in recipe files; opt out an intentional inline block with `<!-- lint-recipes: allow <reason> -->` on the line above the fence.
  Pass explicit positional source paths for the actual repo or files being edited; the runtime location is not the scan target. From a consumer root editing mounted L1, run `node .copilot-toolkit/build/scripts/lint-recipes.mjs .copilot-toolkit/.github`; for self-hosted toolkit source, run `node build/scripts/lint-recipes.mjs .github` from the toolkit root; for a consumer's own tools, run `node .copilot-toolkit/build/scripts/lint-recipes.mjs .github` from that consumer root. Adjust the positional paths for other edited mounts. A no-argument invocation scans only the cached runtime; do not rebuild to gate source edits. This recipe gate does not impose upstream host-marker policy on consumers.
- **Helper scripts default to Node** — a new committed `scripts/` helper is a `.mjs` run via `node`, not a `.ps1`/`.sh`, UNLESS it must run before Node is guaranteed or needs a shell-native capability with no reasonable Node equivalent. Prefer Node; when you pick PowerShell/bash, state the reason in the script header.
...
- **Prompt = self-contained** — prompts contain all runtime instructions. If a prompt needs reference data, inline it or put it in a skill / workflow file. Never load a design document at runtime from the tool's own prompt. Design rationale is not a shipped file: it lives in the commit that made the change and in the issue that motivated it.
...
6. **MANDATORY PR check-in** — create branch, commit, push, create PR via the consumer's configured PR-creation mechanism. Take repo / branch / project / org from the consumer's registry — do NOT hard-code them. IF PR creation fails (auth / tenant mismatch): print a manual URL built from the registry values and stop. Reaching the end of step 6 without an open PR = task NOT done.
...
4. **Root-cause & fix trade-off** — delegate to a subagent: for each finding, return (a) root cause: rule missing / ambiguous / not applied, (b) 2-3 fix options with trade-offs, (c) recommendation. Main agent reviews and picks fixes (ask user if ambiguous).
   - **Before removing "duplication"**: check `git log -p` on the file for prior review fixes that intentionally added this content. If found, it is intentional redundancy — do NOT remove without user confirmation.
5. **Dedup against history** — `git log -p` the tool's files to check whether any finding was reported in a previous review round but not actually fixed. Flag as recurring.
