...
- `## commentCapabilities` — how the platform renders the posted comment: the `pr-review-assemble.mjs comment` meta fields (`wrap` = outer whole-comment `<details open>`; `collapse` = which sections fold, omit = all, `[]` = flat; `sizeBudget` = the platform's comment-size cap). Workflow Step 8 (item 5) copies these into `comment-meta.json`. If a provider omits this section, the assembler defaults apply (`wrap` true, `collapse` all present, `sizeBudget` 60000).
