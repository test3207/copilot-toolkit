...
Change Type: {Step 6}
Repo path: pr-review/{repo}/{prId}/worktree              # isolated git worktree checked out to the PR source branch. Read ALL source files from here -- it reflects the PR head and reading it never disturbs the user's live tree. Do NOT read source from the host repo root. This path is under the self-ignored `pr-review/` tree, so `grep_search` MUST pass `includeIgnoredFiles: true` (scoped to this path) and you open files with `read_file`; `file_search`/`semantic_search` do NOT see the ignored worktree -- use `grep_search`.
