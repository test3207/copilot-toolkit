...
{count} items for author response (tags per tags.md — Severity -> Kind -> Confidence):

- [ ] **[High]** **[Bug]** `file.ts:42` -- Description.
- [ ] **[Medium]** `file.ts:88` -- Description.
- [ ] **[Low]** **[Style]** `file.ts:15` -- Description.
...
Write to `pr-review/{repo}/{prId}/pr-comment.md` (NOT inside `sections/` -- lives one level up so the `review.md` concat in workflow Step 9.1 does not pull it in twice). The `pr-review-assemble.mjs comment` sub-command pulls in the section bodies.

**Collapsible curated comment (since v3.6.0)**: the posted comment is one togglable block per review round. Always-visible = AI header + `## AI Code Review` title + TL;DR (verdict, metrics, validator-curated Action Items). The curated detail sections — Intent and Finding Validation — are each emitted as a nested `<details>` block, collapsed by default, so the reader opens only what they need. The whole comment is wrapped in an outer `<details open>` (default-expanded; click the summary to collapse the entire round — this is what keeps a multi-round PR readable). The raw subagent analyses `20-logic` / `30-impact` / `40-quality` are **NOT** posted — they duplicate each validated finding 2-3x and, being pre-validation, can contradict the Finding Validation verdicts (e.g. show a claim the validator later refuted); they stay in `review.md` for local exploration (`cat pr-review/{repo}/{prId}/review.md`).
...
`comment-meta.json` = `{ "model": "<exact model name from system instructions>", "tool": "<Tool name from SKILL.md Quick Reference, default pr-review>", "version": "<Tool version, e.g. v3.7.0>", "verdict": "<optional; else parsed from 05-tldr.md's **Verdict: ...** line>", "wrap": <bool>, "collapse": [<section keys>] }`. Take `wrap` / `collapse` from the resolved provider's `commentCapabilities` section (defaults when the provider omits it: `wrap` true, `collapse` omitted = collapse every present curated section). The script emits the always-visible header + TL;DR, then each present curated section (`intent`, `validation`) as a collapsed `<details>` block, then the footer, all inside the outer `<details open>`; the raw analyses `20/30/40-*.md` are excluded (they stay in `review.md`). ICM (`90-icm.md`) is NOT posted -- it stays local-only in `review.md` for manual copy-paste into ICM.
