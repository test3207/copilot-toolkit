...
- `toolkit-root` — workspace-relative path the entry prompt resolved (`.copilot-toolkit/build/.github` when consumed via submodule, `build/.github` when self-hosted in this repo). The skill threads this value to every subagent so each one can locate its `{toolkit-root}/skills/work/...` references at runtime.
- `itemId` — numeric tracker id (already resolved by the entry prompt via `.copilot-toolkit/build/scripts/parse-input.mjs`), OR `null` when the caller invoked `/work start` with no id.
